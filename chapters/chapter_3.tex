...
（1）逻辑视图。图\ref{fig:system_logic_view}为平台的逻辑视图，用于从业务能力与数据能力角度描述模块协作关系。平台在逻辑上由内容管理、内容应用、内容治理与数据洞察四个核心域构成：内容管理负责空间与应用组织、内容生产维护、权限控制与审批发布，形成对外一致的“发布口径”；内容应用面向门户与管理端提供展示与检索能力，并面向智能问答场景提供基于检索增强生成的回答与引用溯源；内容治理围绕重复、歧义与覆盖等问题开展检测与任务化流转，推动内容修订与复检回收；数据洞察承接访问、检索与问答链路的事件采集与指标查询，为效果评估与治理决策提供数据支撑。四个核心域围绕统一内容资产与事件机制形成“生产—发布—应用—治理—评估”的闭环。
...
（2）开发视图。图\ref{fig:system_dev_view}为平台的开发视图，用于描述系统在工程实现层面的模块划分与依赖关系。平台对外提供管理后台与内容门户等入口，后端按领域拆分为内容管理、内容应用、内容治理与数据洞察等模块，并复用组织体系、类目与标签、审批流转、权限校验等通用能力。读链路在统一鉴权与范围配置约束下提供列表、详情、检索与问答接口；写链路在内容发布后通过消息队列触发索引更新与统计回收等异步处理，内容治理侧亦可在任务处理后推动内容修订并进入再次发布流程；数据洞察侧承接前端与服务端事件，向下对接数仓侧口径沉淀并对外提供指标查询与下钻能力。
...
（3）物理视图。图\ref{fig:system_physical_view}为平台的物理视图，用于描述部署与运行环境中的组件构成。后端以服务化方式提供内容管理、内容应用、内容治理与数据洞察能力。数据存储由关系型数据库Mysql承载组织、内容元数据、权限与任务等结构化数据，文档数据库MongoDB用于承载灵活结构字段；内存数据库Redis用以支持缓存、分布式锁等能力；检索侧同时使用全文检索引擎Elasticsearch与向量数据库Milvus支撑关键词召回与语义召回；消息队列RocketMQ用于解耦发布后的索引更新与统计回收等异步链路；数据洞察侧对接数仓Hive沉淀的宽表与明细表，为前端可视化与下钻分析提供统一数据来源。
