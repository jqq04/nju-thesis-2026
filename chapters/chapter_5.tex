...
为验证平台在高频浏览、检索与问答场景下的可用性，本文对内容列表、内容检索、内容详情与智能问答接口进行分级压测。测试采用固定发布数据集与预热后的索引环境，逐步提升并发请求强度，并统一观察平均响应时间、P95、P99、错误率等指标。测试结果如表\ref{tab:perf_scope}所示，结果表明，页面化读接口在较高并发下仍能保持较低时延与错误率，而智能问答链路虽然因涉及检索增强与大模型生成导致时延明显更高，但整体仍处于可接受范围内。
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.16\linewidth}|c|c|c|c|>{\centering\arraybackslash}p{0.12\linewidth}|X}
    \hline
    ...
    智能问答 & 100 & 2720 & 3960 & 5110 & 0.89\% & 流式输出的首包时延 \\ \hline
  \end{tabularx}
\end{table}
...
为评估系统在内容规模增长时的扩展性，本文进一步从已发布内容规模为10万级与100万级两类场景出发，观察全文索引、向量索引以及服务侧关键资源的变化趋势。相关结果表如表\ref{tab:resource_scale}所示,结果表明，当内容规模从10万级增长到100万级时，索引存储与服务资源消耗整体呈近似线性增长，检索时延虽有所上升，但系统仍具备较好的扩展性。
...
本章对内容应用与管理平台的测试环境、单元测试、功能测试以及非功能与性能测试进行了说明。测试结果表明：平台在组织与权限边界、内容发布口径、事件驱动索引更新、RAG问答引用可追溯以及治理任务线上化闭环等关键链路上能够满足预期；在非功能维度上，平台较好地满足了一致性、安全隔离、可扩展性与稳定性要求；在性能维度上，系统在典型业务负载下，仍能保证较好的性能水平和资源消耗。
